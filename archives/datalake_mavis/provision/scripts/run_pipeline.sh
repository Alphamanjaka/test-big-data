#!/bin/bash

# Pipeline ELT : RAW -> SILVER (FHIR) -> GOLD
# Les scripts utilisent des imports relatifs : exécution en module (-m)
# depuis la racine du projet.
# Chaque étape s'arrête en cas d'erreur (pas de "succès" mensonger).

PROJECT_ROOT="/home/vagrant/datalake-mavis"
LOG_DIR="$PROJECT_ROOT/provision/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/elt.log"
cd "$PROJECT_ROOT" || exit 1

DATE=$(date '+%Y-%m-%d %H:%M:%S')

echo "============================================================" >> "$LOG_FILE"
echo "🚀 Lancement du pipeline ELT à $DATE" >> "$LOG_FILE"
echo "============================================================" >> "$LOG_FILE"

run_step() {
    local desc="$1"; shift
    echo "[STEP] $desc" >> "$LOG_FILE"
    "$@" >> "$LOG_FILE" 2>&1
    local rc=$?
    if [ $rc -ne 0 ]; then
        END_DATE=$(date '+%Y-%m-%d %H:%M:%S')
        echo "❌ ÉCHEC (code $rc) à l'étape : $desc — pipeline arrêté à $END_DATE" >> "$LOG_FILE"
        echo "------------------------------------------------------------" >> "$LOG_FILE"
        echo ""
        echo "❌ Pipeline interrompu ($rc) : $desc — voir $LOG_FILE"
        exit $rc
    fi
}

run_step "[1/4] 🧩 Extraction des données brutes (RAW)" \
    /usr/bin/python3 -m provision.scripts.ELT.gen_extract_raw

run_step "[2/4] Carte de correspondance tables/colonnes -> FHIR" \
    /usr/bin/python3 -m provision.scripts.ELT.gen_fhir_mapping

run_step "[3/4] 🧬 Transformation SILVER (FHIR harmonisé)" \
    /usr/bin/python3 -m provision.scripts.ELT.create_silver

run_step "[4/4] 📊 Création de la couche GOLD (analytique)" \
    /usr/bin/python3 -m provision.scripts.ELT.create_gold

END_DATE=$(date '+%Y-%m-%d %H:%M:%S')
echo "✅ Pipeline ELT FHIR terminé avec succès à $END_DATE" >> "$LOG_FILE"
echo "------------------------------------------------------------" >> "$LOG_FILE"
echo ""
echo "✅ Pipeline ELT complet : RAW -> SILVER -> GOLD OK"
